...
public class LoginController {
    ...
    public static Handler handleLoginPost = ctx -> {
        ...
        if (!UserController.authenticate(getQueryUsername(ctx), getQueryPassword(ctx))) {
        	...
        	Account account = AccountDAO.getUserDetailsByUsername(getQueryUsername(ctx));
        	...
        	else if(account.getType().equalsIgnoreCase("users") || account.getType().equalsIgnoreCase("critics"))
        	{
	        	ctx.sessionAttribute("currentUser", getQueryUsername(ctx));
	            model.put("authenticationSucceeded", true);
	            model.put("currentUser", getQueryUsername(ctx));
        	}
        	else if(account.getType().equalsIgnoreCase("pco"))
        	{
	        	ctx.sessionAttribute("pcoUser", getQueryUsername(ctx));
	            model.put("authenticationSucceeded", true);
	            model.put("pcoUser", getQueryUsername(ctx));
        	}
    ...
    public static Handler handleLogoutPost = ctx -> {
        ...
        ctx.sessionAttribute("pcoUser", null);
